  ...
  fn format_uri(&self, uri: &Uri) -> String {
    ...
    for (prefix, namespace_uri) in self.namespaces.iter() {
      if uri.to_string().starts_with(namespace_uri.to_string()) {
        ...
        let path = uri.to_string().to_owned().replace(namespace_uri.to_string(), "").replace("/", ":");
...
mod tests {
  ...
  fn test_turtle_qname_node_formatting() {
    ...
    hashmap.insert("example".to_string(), Uri::new("http://example.org/".to_string()));

    let formatter = TurtleFormatter::new(&hashmap);
    let node = Node::UriNode { uri: Uri::new("http://example.org/show/localName".to_string()) };

    assert_eq!(formatter.format_node(&node), "example:show:localName".to_string());
  }
  ...
  fn test_turtle_boolean_literal_node_formatting() {
    ...
    assert_eq!(formatter.format_node(&node), "true^^<http://www.w3.org/2001/XMLSchema#boolean>".to_string());
  }
  ...
  fn test_turtle_integer_literal_node_formatting() {
    ...
    assert_eq!(formatter.format_node(&node), "123^^<http://www.w3.org/2001/XMLSchema#integer>".to_string());
  }
  ...
  fn test_turtle_decimal_literal_node_formatting() {
    ...
    assert_eq!(formatter.format_node(&node), "123.123^^<http://www.w3.org/2001/XMLSchema#decimal>".to_string());
  }
}
