...
pub trait TokensFromTurtle<R: Read>: TokensFromNTriples<R> {
  ...
  fn get_prefix_directive(input_reader: &mut InputReader<R>) -> Result<Token> {
    ...
    let mut name = input_reader.get_until_discard_leading_spaces(|c| c == ':')?.to_string();
    name.push(':');
  ...
  fn get_numeric(input_reader: &mut InputReader<R>) -> Result<Token> {
    let numeric = input_reader.get_until_discard_leading_spaces(InputReaderHelper::node_delimiter)?;
      ...
      match input_reader.peek_until(InputReaderHelper::node_delimiter) {
        Ok(mut input_chars) => {
          ...
          if TurtleSpecs::is_double_literal(&complete_numeric.to_string()) {
            let _ = input_reader.get_until_discard_leading_spaces(InputReaderHelper::node_delimiter)?; // consume
  ...
  fn get_boolean_literal(input_reader: &mut InputReader<R>) -> Result<Token> {
    let boolean = input_reader.peek_until_discard_leading_spaces(InputReaderHelper::node_delimiter)?;
  ...
  fn get_literal(input_reader: &mut InputReader<R>) -> Result<Token> {
    ...
    while !found_literal_end {
      literal.push_str(&input_reader.get_until(|c| c == literal_delimiter.unwrap())?.to_string());
    ...
    match input_reader.peek_next_char()? {
      ...
      Some('^') => {
        ...
        match input_reader.peek_next_char()? {
          ...
          Some(_) => {
            match Self::get_qname(input_reader)? {
...
impl<R: Read> RdfLexer<R> for TurtleLexer<R> {
  ...
  fn get_next_token(&mut self) -> Result<Token> {
    ...
    match try!(self.input_reader.peek_next_char_discard_leading_spaces()) {
      ...
      Some('"') | Some('\'') => return <TurtleLexer<R> as TokensFromTurtle<R>>::get_literal(&mut self.input_reader),
      ...
      Some('.') => {
        ...
        return TurtleLexer::get_numeric(&mut self.input_reader).or_else(|_| {
      ...
      Some(c) if InputReaderHelper::digit(c) => return TurtleLexer::get_numeric(&mut self.input_reader),
      Some(_) => {},
