...
use sparql::pattern::{Pattern, GroupPattern, NodePattern, TriplePattern};
use node::Node;
use uri::Uri;
...
impl<R: Read> SparqlParser<R> {
  ...
  pub fn decode(&mut self) -> Result<SparqlQuery> {
    ...
    Err(Error::new(ErrorType::InvalidToken,
  ...
  pub fn read_select_query(&mut self) -> Result<SparqlQuery> {
      ...
      Token::Asterisk => {
        match query_type.clone() {
          SparqlQueryType::SelectDistinct =>
            query_type = SparqlQueryType::SelectAllDistinct,
          SparqlQueryType::SelectReduced =>
            query_type = SparqlQueryType::SelectAllReduced,
          _ =>
            query_type = SparqlQueryType::SelectAll
        }
      },
    ...
    // parse WHERE clause
    match self.lexer.peek_next_token()? {
      Token::Where => {   // WHERE keyword is optional but always followed by a group
        let _ = self.lexer.get_next_token();
      },
      _ => {}
    }
  ...
  fn read_predicate_object_list_pattern(&mut self, subject: &NodePattern, query: &mut SparqlQuery) -> Result<Vec<TriplePattern>> {
  ...
  fn read_predicate_with_object_pattern(&mut self, query: &mut SparqlQuery) -> Result<(NodePattern, NodePattern)> {
    // read the predicate
    let predicate = match self.lexer.get_next_token()? {
      Token::Uri(uri) => NodePattern::FixedNode(Node::UriNode { uri: Uri::new(uri) }),
      Token::KeywordA => NodePattern::FixedNode(Node::UriNode { uri: RdfSyntaxDataTypes::A.to_uri() }),
      Token::QName(prefix, path) => {
        let mut uri = query.get_namespace_uri_by_prefix(prefix)?.to_owned();
        uri.append_resource_path(path.replace(":", "/"));   // adjust the QName path to URI path
      ...
      _ => return Err(Error::new(ErrorType::InvalidToken, "Invalid token for SPARQL triple pattern predicate."))
    };
  ...
  fn read_object_pattern(&mut self, query: &mut SparqlQuery) -> Result<NodePattern> {
    match self.lexer.get_next_token()? {
      ...
      Token::LiteralWithLanguageSpecification(literal, lang) =>
        Ok(NodePattern::FixedNode(Node::LiteralNode { literal: literal, data_type: None, language: Some(lang) })),
      Token::LiteralWithUrlDatatype(literal, datatype) =>
        Ok(NodePattern::FixedNode(Node::LiteralNode { literal: literal, data_type: Some(Uri::new(datatype)), language: None })),
      Token::Literal(literal) =>
        Ok(NodePattern::FixedNode(Node::LiteralNode { literal: literal, data_type: None, language: None })),
      _ => Err(Error::new(ErrorType::InvalidToken, "Invalid token for SPARQL object pattern."))
    }
  }
